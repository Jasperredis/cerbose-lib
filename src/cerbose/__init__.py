...
def INTERNAL_getTypeItem(item, config):
    try:
        return {"label": config['tags']['text'][item],  "colour": SUBSITUTE[config['tags']['colours'][item]]}
    except Exception as e:
        INTERNAL_iprint("error", f"CERBOSE: {e}")

def INTERNAL_configIsValid(config):
    ...
    for item in MAIN_KEYS:
        if item not in config:
            INTERNAL_iprint("error", f"defconf: configIsValid: Invalid configuration was passed. Missing key: {item}")
            return False
...
def INTERNAL_getCin(options, cprint, lower):
    gotans = False
    while not gotans:
        ans = input("> ")
        if lower:
...
# Different types
TYPES = {'meta': {'program': 'Cerbose', 'author': 'jasperredis'}, 'tags': {'colours': {'none': 'white', 'ok': 'green', 'note': 'cyan', 'warn': 'yellow', 'error': 'red', 'debug': 'magenta', 'info': 'cyan', 'input': 'lightblue', 'load': 'red', 'pause': 'yellow', 'stat': 'magenta', 'fatal': 'red', 'trace': 'magenta', 'proc': 'magenta'}, 'text': {'none': 'NONE', 'ok': 'OK', 'note': 'NOTE', 'warn': 'WARN', 'error': 'ERROR', 'debug': 'DEBUG', 'info': 'INFO', 'input': 'INPUT', 'load': 'LOAD', 'pause': 'PAUSE', 'stat': 'STAT', 'fatal': 'FATAL', 'trace': 'TRACE', 'proc': 'PROC'}, 'symbols': {'bracket left': '[', 'bracket right': ']', 'text divisor': ':'}}, 'cerbar': {'bracket left': '[', 'bracket right': ']', 'fill symbol': '#', 'empty symbol': '-'}, 'space-repeat-tolerance': 5, 'timeformat': '%H:%M:%S'}
...
def cprint(type, text, *, logfile=None, logfeedback=False, textcol="normal", stagtype=None, timestamp=False, valonly=False):
    ...
    if type == "": # Blank
        ...
        tagcol = TYPES["tags"]["colours"].get(type, TYPES["tags"]["colours"]["none"]) # Fallback to NONE
        tagtxt = TYPES["tags"]["text"].get(type, TYPES["tags"]["text"]["none"]) # Fallback to NONE
        labellength = 0
        # Add timestamp
        if timestamp:
        ...
        # Add stagtype
        if stagtype != None:
            stagcol = TYPES["tags"]["colours"].get(stagtype, TYPES["tags"]["colours"]["none"]) # Fallback to NONE
            stagtxt = TYPES["tags"]["text"].get(stagtype, TYPES["tags"]["text"]["none"]) # Fallback to NONE
            labellength += len(f"{symbols['bracket left']}{stagtxt}{symbols['bracket right']}")
        labellength += len(f"{symbols['bracket left']}{tagtxt}{symbols['bracket right']}{symbols['text divisor']}")
        # Do alignment
        if labellength > align:
            ...
            spacerepeatcount += 1
            if spacerepeatcount >= TYPES['space-repeat-tolerance'] and align > labellength:
                align = labellength
                spacerepeatcount = 0
        # Make padding
    ...
    if textcol in INTERNAL_SUBSITUTE:
        ...
        ctext = Fore.RESET
        INTERNAL_iprint("warn", "CERBOSE: cprint: Provided texcol colour is not a valid colour.")
    # Make output
    if valonly:
        return backer + text
    else:
    ...
    if logfile is not None:
        try:
            with open(logfile, "a") as f:
                ...
                if stagtype != None:
                    writecont += f"{symbols['bracket left']}{stagtxt}{symbols['bracket right']}"
...
def mprint(type, text, *, logfile=None, logfeedback=False, textcol="normal", stagtype=None, timestamp=False, valonly=False):
    global align
    symbols = TYPES["tags"]["symbols"]
    text = text.split('\n')
    tagtxt = TYPES["tags"]["text"].get(type, TYPES["tags"]["text"]["none"]) # Fallback to NONE    
    ...
    cprint(type, text, logfile=logfile, logfeedback=logfeedback, textcol=textcol, stagtype=stagtype, timestamp=timestamp, valonly=valonly)
...
def cin(type, text, options, *, log=False, logfile=None, logfeedback=False, textcol="normal", stagtype=None, timestamp=False, lower=False, showop=False):
    ...
    if showop and options != 'any': # Print options (really just get them in memory)
        ...
        for item in options:
            count += 1
            printext += f"\n{count}) {item}"
    mprint(type, printext, logfile=logfile, logfeedback=logfeedback, textcol=textcol, stagtype=stagtype, timestamp=timestamp) # Actually print options
    return INTERNAL_getCin(options, cprint, lower) # Function for simplicity sake
...
def defconf(type, content):
    ...
    if type == "f" or type == "file": # Handle files
        ...
        if isinstance(content, str): # Strings
            ...
            INTERNAL_iprint("error", f"defconf: Using input mode but inputted config is not a string or dict.")
            return None
    if not INTERNAL_configIsValid(CERBOSE_CONFIG): # Verify all keys are present
